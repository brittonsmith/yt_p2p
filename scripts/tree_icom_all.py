...
if __name__ == "__main__":
    ...
    for it in range(len(trees)):
        ...
        n_nodes = (tree['forest', 'mass'] >= m_min).sum()
        if n_nodes == 0:
            continue
        if yt.is_root():
            yt.mylog.info(f'Assembling {n_nodes} for tree {it}/{len(trees)}.')
        ...
        for my_storage, inode in yt.parallel_objects(range(len(nodes)), dynamic=True, storage=storage):
            ...
            yt.mylog.info(f'Finished node {inode}/{len(nodes)} of tree {it}/{len(trees)}.')

        if not nodes:
            continue

        if yt.is_root():
            yt.mylog.info(f'Assembling results for tree {it}/{len(trees)} ({tree}).')
